...
const BrandPage: React.FC<BrandPageProps> = ({ params }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        setError(`Failed to fetch products for brand: ${name}.`);
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="flex flex-col items-center p-4 w-full max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-6 text-white">Brand: {decodeURIComponent(name)}</h1>
      ...
      <div className="w-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-6 rounded-2xl shadow-2xl border border-gray-700/50">
        <h2 className="text-2xl font-bold mb-4 text-white">Products from this Brand</h2>
